...
export const upvotePostCommentHandler = function (schema, request) {
	...
	try {
		...
		post.comments[commentIndex].votes.downvotedBy = post.comments[commentIndex].votes.downvotedBy.filter(
...
export const downvotePostCommentHandler = function (schema, request) {
	...
	try {
		...
		if (post.comments[commentIndex].votes.downvotedBy.some((currUser) => currUser._id === user._id)) {
			return new Response(400, {}, { errors: ["Cannot downvote a post that is already downvoted. "] });
		}
		post.comments[commentIndex].votes.upvotedBy = post.comments[commentIndex].votes.upvotedBy.filter(
			(currUser) => currUser._id !== user._id
		);
