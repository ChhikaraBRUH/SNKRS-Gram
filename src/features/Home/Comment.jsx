...
export function Comment({ comment, postId }) {
	...
	const userInfo = allUsers && allUsers?.find((commentUser) => comment.username === commentUser.username);
	...
	return (
		<div className='flex gap-3'>
			<img
				...
				onClick={() => (comment.username === user.username ? navigate("/profile") : navigate(`/user-profile/${userInfo?.userHandler}`))}
			/>
			<div className='bg-slate-200 rounded-xl py-1 px-3 grow'>
				<div className='flex justify-between h-6'>
					<span
						...
						onClick={() =>
							comment.username === user.username ? navigate("/profile") : navigate(`/user-profile/${userInfo?.userHandler}`)
						}>{`${userInfo?.firstName} ${userInfo?.lastName}`}</span>
					{comment.username === user.username && (
						<div className='rounded-full px-2 cursor-pointer relative' onClick={() => setEditModal(!editModal)}>
							<i className={`text-sm fa-solid fa-ellipsis-vertical opacity-60 hover:opacity-100 ${isEdit && "hidden"}`} />
							{editModal && (
								<ul className='dropdown absolute m-0 text-sm px-1 py-1 rounded-lg top-4 right-4 bg-white w-30 gap-1'>
									...
									<li
										className='hover:bg-slate-200 flex items-center px-1 py-1 rounded-md'
										onClick={() => dispatch(deleteComment({ postId: postId, commentId: comment._id }))}>
				...
				{isEdit ? (
					<form className='self-center border-gray-400 grow flex space-between items-center rounded-md pb-1' onSubmit={() => editHandler()}>
						<input
							className='text-sm grow focus:outline-none bg-slate-200 text-gray-600 border-b-2 border-gray-400 mr-4'
							value={inputComment}
							onChange={(e) => setInputComment(e.target.value)}
						/>
						<i className='fa fa-times-circle-o cursor-pointer text-gray-500' aria-hidden='true' onClick={() => setIsEdit(false)} />
						<i
