...
export function Home() {
	...
	useEffect(() => {
		if (allPosts) {
			setFeedPost(
				allPosts
					?.filter((post) => post.username === user.username || user.following.find((ele) => post.username === ele.username))
	...
	const trendingHandler = () => {
		...
		setTrendPost((prev) => ({
			...prev,
			posts: [...feedPost].sort((a, b) => b.likes.likeCount - a.likes.likeCount).filter((post) => post.likes.likeCount > 0),
		}));
	};
	...
	return (
		<div className={`home-container w-2/4  md:w-full md:mx-4 `}>
			<div className='home-input bg-white px-4 py-3 rounded-xl gap-4 mb-8 text-center border-b-2 border-violet-500 font-semibold'>FEED</div>
			<div
        ...
        flex justify-between items-center px-4 py-3 rounded-xl gap-4 mb-8'>
         ...
         px-4 py-2 rounded-xl gap-4 mb-6 text-center  font-semibold justify-evenly text-gray-500 '>
				<div className={`w-1/2 border-r-2 border-violet-500 ${trendPost.isTrend ? "text-violet-500" : ""}`}>
					<span className='hover:opacity-70 cursor-pointer' onClick={() => trendingHandler()}>
						<i className='fa-solid fa-fire pr-2' />
						Trending
					</span>
				</div>
				<div className={`w-1/2 ${!trendPost.isTrend ? "text-violet-500" : ""}`}>
			...
			{trendPost.isTrend ? (
				<div className='flex flex-col gap-6'>
					{trendPost.posts.length !== 0 ? (
						...
						<p className='bg-white px-4 py-12 rounded-xl gap-4 text-center border-2 border-violet-500 font-semibold text-xl'>
							Like some posts to see them trending!
						</p>
					)}
				</div>
			) : (
				<div className='flex flex-col gap-6'>
					{feedPost.length !== 0 ? feedPost.map((post) => <SinglePost key={post._id} post={post} />) : <></>}
				</div>
			)}
		</div>
	);
