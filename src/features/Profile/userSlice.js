...
export const followUnFollowUser = createAsyncThunk("post/followUnFollowUser", async ({ userId, dispatch, isFollow }, thunkAPI) => {
	try {
		const token = localStorage.getItem("token");
		const response = !isFollow ? await unFollowUserService(token, userId) : await followUserService(token, userId);
